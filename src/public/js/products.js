...
async function showPurchaseButton() {
	const res = await fetch(`http://localhost:4000/api/carts/${cartId}`);
...
async function addProduct(cartId, pid) {
	const res = await fetch(`http://localhost:4000/api/carts/${cartId}/product/${pid}`, {
		method: 'POST',
		body: JSON.stringify({}),
	});
	const data = await res.json();
...
previousButton.addEventListener('click', async () => {
	page--;
	await getProducts('http://localhost:4000/api/products', page);
});
...
logoutButton.addEventListener('click', async () => {
	const res = await fetch('http://localhost:4000/api/session/logout');
	const data = await res.json();
...
purchaseButton.addEventListener('click', async () => {
	const res = await fetch(`http://localhost:4000/api/carts/${cartId}/purchase`, {
		method: 'POST',
		body: JSON.stringify({}),
	});
	const data = await res.json();
...
getProducts('http://localhost:4000/api/products', page);
